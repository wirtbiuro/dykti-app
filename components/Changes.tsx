...
import { getStepProps, isStepPropErrors, getEdgeDayMillis, getStepPropValue } from '../utilities'
...
const Changes: FC<IChangesProps> = ({ step, prevStep }) => {
    const getChanges = () => {
        ...
        const _stepProps = stepProps
            ...
            .filter((prop) => {
                // console.log('prop.value', prop.view)
                const prevStepValue = prevStep ? getStepPropValue(prevStep, prop.key) : null
                // console.log({ prevStepValue })

                return (prevStep && prop.view !== prevStepValue) || (!prevStep && prop.value !== null)
            })
