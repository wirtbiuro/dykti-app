...
const Orders: FC<IOrderProps> = ({ orders, stepName, children }) => {
    const [viewingFormOrderId, setViewingFormOrderId] = useState<number | null>()
                ...
                return (
                    <StepStyled key={order.id}>
                        ...
                        {viewingFormOrderId !== order.id && (
                            <button onClick={() => setViewingFormOrderId(Number(order.id))}>Kontynuować</button>
                        )}
                        {viewingFormOrderId === order.id && (
                            <button onClick={() => setViewingFormOrderId(Number(null))}>Zamknij</button>
                        )}
                        {shouldConfirmView && (
                            <button
                                onClick={() =>
                        ...
                        {historyOrderId !== order.id && (
                            <button onClick={() => setHistoryOrderId(order.id)}>Pokaż historię zmian</button>
                        )}
                        {historyOrderId === order.id && (
                            <button onClick={() => setHistoryOrderId(null)}>Zamknij histoię zmian</button>
                        )}
